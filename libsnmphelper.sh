...
snmp_switch_mac_address() {
	...
	switch_iface_id=$(snmpget -v2c -c 'cclubswitches' -ObentUq $ip $tmp_OID)
        switch_iface_id=${switch_iface_id:${#tmp_OID}}
	tmp_OID=".1.3.6.1.2.1.2.2.1.6.$switch_iface_id "
	switch_mac_address=$(snmpget -v2c -c 'cclubswitches' -ObentUq $ip $tmp_OID)
...
snmp_strip_results () {
	...
	tmp_snmp_readout=$(snmpwalk -v2c -c 'cclubswitches' -ObentUq $ip $oid);
	OLDIFS=$IFS;
	IFS=$'\n';
        for item in $tmp_snmp_readout; do
                IFS=' '
